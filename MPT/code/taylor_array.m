clear all;close all;
%泰勒法综合阵列，阵元数为奇数
R0=30;  %the level difference of main lobe and sidelobes db
b=10^(R0/20);
A=acosh(b)/pi;%a parpmeter in the null expression of the ideal space factor
nb=floor(2*A^2+0.5)+5;%the first nb nulls of S is the same with the dieal space factor ,and n1>=2*A^2+0.5
n1=1:nb-1;
delta=nb/sqrt(A^2+(nb-0.5)^2);%expanding factor
u0=delta*sqrt(A^2+(n1-0.5).^2);%the positive nulls of the ideal space factor
N=15;%the antenna number of the array
theta0=pi*90/180;
theta=0:pi/2000:pi;
lemda=1;
d=lemda/2;
...
%     S1=S1+2*I(n+1)*exp(j*2*pi*d*n*u/L);
end
angle = 0:180/2000:180;
...
plot(u,Slog1-Slog);
figure(3)
plot(angle,Slog,'k',angle,Slog1,'r')
grid on